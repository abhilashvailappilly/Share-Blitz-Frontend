...
interface connectionsInterface {
    followers: user[]; // Assuming any type for followers, adjust as needed
    followings: user[]; // Assuming any type for followings, adjust as needed
  }
  ...
  return (
    <>
            ...
            <div className="flex justify-evenly">
              <div onClick={()=>setOpenFollowers(true)} className="w-36 h-10 mt-3 mb-2 bg-gray-200 flex justify-center items-center font-bold border-2 border-gray-400 rounded-xl ">
              ...
              <div onClick={()=>setOpenFollowings(true)} className="w-36 h-10 mt-3 mb-2 bg-gray-200 flex justify-center items-center font-bold border-2 border-gray-400 rounded-xl ">
                <h1 className="m-2">{userConnections?.followings.length || 0}</h1>
                <h2>Followings</h2>
              </div> 
            </div>
            {openFollowers && <FollowingsModal isFriend={isFriend} isPrivate={isPrivate} setOpen={setOpenFollowers} title={"Followers"} followings={userConnections?.followers || []}/>}
            {openFollowings && <FollowingsModal isFriend={isFriend} isPrivate={isPrivate} setOpen = {setOpenFollowings} title= { "Followings"} followings={userConnections?.followings || []}/>}
    </>
  )
}
